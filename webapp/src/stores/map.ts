...
const hot = L.tileLayer(
  ...
  {
    attribution:
      '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors; Humanitarian map style by <a href="https://www.hotosm.org/">HOT</a>',
  }
);

const geoserverWorkspaces = [
  "Citizen_Participation",
  "BaseData",
  "FlowPaths",
  "LandUse",
  "Massnahmen_Ergebnisse",
  "Wadi",
  "vector",
];
...
export const useMapStore = defineStore("map", () => {
  ...
  const initializeMap = (containerId: string, options: L.MapOptions) => {
    ...
    toRaw(map.value).createPane("vector");
    toRaw(map.value).getPane("vector").style.zIndex = 300;

    toRaw(map.value).createPane("raster");
    toRaw(map.value).getPane("raster").style.zIndex = 200;
    /* Map legend */
    legend.value = L.control.legend({ position: "bottomleft" });
    legend.value.addTo(toRaw(map.value));

    map.value.on("overlayadd", (event: L.LayerEvent & { name: string }) => {
      legend.value?.toggleLegendForLayer(
        event.layer as L.TileLayer.WMS,
        true,
        event.name
      );
    });
    ...
    toRaw(
      map.value.on(
        "overlayremove",
        (event: L.LayerEvent & { name: string }) => {
          legend.value?.toggleLegendForLayer(
            event.layer as L.TileLayer.WMS,
            false
          );
    ...
    /* Scale bar */
    L.control
      .scale({ maxWidth: 300, position: "bottomright" })
      .addTo(toRaw(map.value));

    hot.addTo(toRaw(map.value));
  };
  ...
  const initializeLayers = async () => {
    ...
    const layerControl: any = L.control
      .groupedLayers(
        {},
        {},
        {
    ...
    // ************************************************************************
    // Raster workspace'ünü belirle
    const rasterWorkspace = "raster";
    // Raster workspace altındaki raster katmanlarını al
    const rasterLayers = await geoserver.GetRasterLayers(rasterWorkspace);
    for (const layerInfo of rasterLayers) {
      const wms = await RasterWmsToLayer(rasterWorkspace, layerInfo);
      wms.bringToBack();
      overlayMaps.value[layerInfo.name] = wms;
      layerControl.addOverlay(wms, layerInfo.name, rasterWorkspace);
    }
    // ************************************************************************
    dmpLayerGroup.value = L.layerGroup();
  };
  // ************************************************************************
  const RasterWmsToLayer = (workspace: string, layerInfo: any) => {
    const wms = L.tileLayer.wms(getWmsBaseUrl(workspace), {
  ...
  const addDMP = async (layerName: string, popupContent: string) => {
    const layerInfo = await geoserver.GetFeatureType(
  ...
  const wmsToLayer = (
    workspace: string,
    layerInfo: GS.WMSLayerInfo | GS.FeatureTypeInfo
  ) => {
    const wms = L.tileLayer.wms(getWmsBaseUrl(workspace), {
      layers: layerInfo.name,
      format: "image/png",
