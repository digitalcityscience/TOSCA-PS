...
app.delete("/masterplans/:id", async (req, res) => {
  try {
    ...
    res
      .status(404)
      .send(`Error: masterplan with ID ${req.params.id} not found`);
    return;
  }
  ...
  const publicreviews = await db
    ...
    .find({ masterplanId: new ObjectId(req.params.id) })
    .toArray();

  for (const publicreview of publicreviews) {
    await db
      .collection("objections")
      .deleteMany({ publicReviewId: publicreview._id });
  }
  await db
    .collection("publicreviews")
    .deleteMany({ masterplanId: new ObjectId(req.params.id) });
...
app.post("/publicreviews", jsonParser, async (req, res) => {
  ...
  if (
    !(await db
      .collection("masterplans")
      .findOne({ _id: new ObjectId(req.body.masterplanId) }))
  ) {
    res
      .status(404)
      .send(`Error: masterplan with ID ${req.body.masterplanId} not found`);
    return;
  }
...
app.post("/publicreviews/:id/objections", jsonParser, async (req, res) => {
  ...
  if (
    ...
    res
      .status(404)
      .send(`Error: public review with ID ${req.params.id} not found`);
    return;
  }
...
app.post(
  "/objections/:id/attachments",
  uploadParser.array("attachment"),
  async (req, res) => {
    try {
      ...
      res
        .status(404)
        .send(`Error: objection with ID ${req.params.id} not found`);
      return;
    }
